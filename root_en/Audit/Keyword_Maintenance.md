# Keyword_Maintenance

**Keyword Category Setting, Add Keyword**

  #@img_AD0006/image118.png

**Keyword Category Setting：可依據不同分類設定關鍵字、 and DepartmentperformVerify。**

  #@img_AD0006/image119.png

**Keyword Category Modification Maintenance [Edit Icon]**

  #@img_AD0006/image120.png

**Add Keyword: Different keywords, scores, and counting rules can be set under keyword category.**

  #@img_AD0006/image121.png

**累計: 若設定為Y，表示風險分數 = 關鍵字出現次數N \* 分數**

**Regex: If set to Y, keyword is detected by regex; following are two examples:**

**ID Number: \[A-Z\]\[12\]\\d{8}**

**cell phoneNumber: 09\\d{8}**

**修改關鍵字：選擇要修改的關鍵字，點擊 [Edit Icon]** **直接performEdit。**

  #@img_AD0006/image122.png
