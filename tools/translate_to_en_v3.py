...
def translate_content(content):
    ...
    for k, v in CONTENT_MAP.items():
        if k.startswith("r") and len(k) > 1: # naive regex detection
             # It's a regex if I defined it with r"" in the map logic, 
             # but here keys are strings. I used r"..." in source code but they are generic strings in dict.
             # I will treat keys starting with !\[ as distinct logic or just use string replace for most.
             pass
    
    # Let's simple string replace for stability, descending by length of key to avoid partial matches
...
def update_links(content):
    ...
    # Pattern: [Link Text](../../root/OldDir/OldFile.md)
    # We need to find these and replace OldDir and OldFile with NewDir and NewFile.
    ...
    def replacer(match):
        ...
        # parts might be ["..", "..", "root", "OldDir", "OldFile.md"]
