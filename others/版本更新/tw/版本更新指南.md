# PSS Powermoat 版本更新指南

## 一、更新前準備

### 1.1 備份PSSDB (以 SQL EXPRESS 為例)

- 開啟　SQL SERVER STUDIO
  
- 開啟　Databases -> PSSDB
  
  #@img_A0006/A0006_0001.png

- PSSDB　操作　->　Tasks -> Back up ,記錄下Back up　路徑

  #@img_A0006/A0006_0002.png

  #@img_A0006/A0006_0003.png

### 1.2 暫停服務

- 開啟　Windows Service 服務管理介面
  
  #@img_A0006/A0006_0004.png

- 關閉以下 Service

   (PSSWEB,PSSBatchJob,PSSBatchJobCore,PSSOCRService,PSSMonitor,PSSSiteBatchJob)

- 開啟　IIS 服務管理介面

  #@img_A0006/A0006_0005.png

- 點選伺服器操作 stop

  #@img_A0006/A0006_0006.png

### 1.3　更新資料庫

- 開啟開裝ISO
- 開啟目錄　\tools\Database\Upgrade
  
  #@img_A0006/A0006_0007.png

- 找出需更新的SQL 檔案(依據版本編號日期進行執行)
  
  #@img_A0006/A0006_0008.png

  #@img_A0006/A0006_0009.png

- 更新資料庫完成

### 1.4　更新主程式

- 開啟安裝ＩＳＯ目錄
- 以管理者開啟 PowerShell
- 執行　pss_system_update.ps1

  #@img_A0006/A0006_0010.png
  
- 點選更新功能,點擊　OK 執行
  
  #@img_A0006/A0006_0011.png

- 執行完後即更新完成

  #@img_A0006/A0006_0012.png

- 啟動 IIS 及　PSS 服務

  #@img_A0006/A0006_0013.png

  #@img_A0006/A0006_0014.png

- 啟動 IIS 及　PSS 服務

### 1.5 程式回滾方式

#### 1.5.1 IIS WEB程式回滾

-　至目錄　C:\inetpub\wwwroot　,　將　PSSWeb　解壓縮, 將檔案覆蓋於　C:\inetpub\wwwroot

#### 1.5.2 Powermoat 服務程式回滾

- 將舊版的服務程式如　(PSSWEB,PSSBatchJob,PSSBatchJobCore,PSSOCRService,PSSMonitor,PSSSiteBatchJob)　重新安裝

#### 1.5.3 資料庫回滾
  
- 將備份的　PSSDB　進行回復
